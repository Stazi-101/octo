...
DEFAULTPATH = 'ocoNew.txt'

WHICHDRAW = (True,False,False) #whether to draw triangles, links, points

'''
Changed from version 1: changed saving/loading format to support
                        future controls
'''
    ...
    def genJelly(self,filename):
        ...
        with open(filename,'r') as theFile:
            ...
            while nextLine:
                ...
                self.nodes.append( Node(real+imag*1j) )
                nextLine = theFile.readline()[:-1]
                
            nextLine = theFile.readline()[:-1]
            while nextLine:
                start,end,length,control = [int(n) for n in nextLine.split(' ')]
                start,end = self.nodes[start], self.nodes[end]
                self.links.append( Link(start,end,length=length,control=control) )
                nextLine = theFile.readline()[:-1]
                
            nextLine = theFile.readline()[:-1]
            while nextLine:
                x,y,z,r,g,b = [int(n) for n in nextLine.split(' ')]
                points = [self.nodes[n] for n in (x,y,z)]
                self.tris.append( Tri(points,(r,g,b) ) )
                nextLine = theFile.readline()[:-1]
